...
def _get_last_month_items(current_ym: str) -> list:
    """获取上个月的预算细项（用于复制方案）"""
...
def _match_scheme(record: dict) -> str:
    """从月度记录的预算金额反推使用的方案"""
    from core.budget import SCHEMES
...
class BudgetTab:
    ...
    def _sort_items(self, items: list) -> list:
        ...
        return sorted(items, key=lambda i: (
            self.CATEGORY_ORDER.get(i.get('category', ''), 99),
            i.get('sort_order', 0),
        ))
        ...
        for i, row in enumerate(self.items_table.rows):
            ...
            for k, v in CATEGORY_LABELS.items():
                if v == cat_text:
                    cat = k
                    break
            weight = self._item_weights[i] if i < len(self._item_weights) else 10
            items.append({
                'category': cat,
                'name': name,
                'weight': weight,
